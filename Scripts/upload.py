...
def release(version, github_access_token):
    ...
    action_path = os.path.join(project_path, 'Jump Desktop Connections.lbaction')

    update_bundle_version(action_path, version)
    sign_bundle(action_path)

    archive_path = archive_bundle(action_path, version)
    html_url = upload_release('nriley/LBJumpDesktop', version, archive_path, github_access_token)
